...
public class Nino extends javax.swing.JFrame {
        private JTextField txtNombre, txtPaterno, txtMaterno, txtEdad, txtTutor, txtTelefono, txtEscuela;
        private static List<Ninoo> listaNinos = new ArrayList<>();
    ...
    public Nino() {
        ...
        JLabel subtitulo = new JLabel("REGISTRO DE ASISTENTES NINOS", SwingConstants.CENTER);
        ...
        lblImagen.setIcon(new ImageIcon(getClass().getResource("/imagenes/NNCorriendo.png")));
        ...
        lblTutor.setFont(new Font("Palatino Linotype", Font.PLAIN, 20));
        txtTutor = new JTextField(40);
        gbc.gridx = 0; gbc.gridy = 4;
        panelCampos.add(lblTutor, gbc);
        gbc.gridx = 1;
        panelCampos.add(txtTutor, gbc);

        JLabel lblTelefono = new JLabel("Teléfono: ");
        lblTelefono.setFont(new Font("Palatino Linotype", Font.PLAIN, 20));
        ...
        lblEscuela.setFont(new Font("Palatino Linotype", Font.PLAIN, 20));
        txtEscuela = new JTextField(40);
        gbc.gridx = 0; gbc.gridy = 6;
        panelCampos.add(lblEscuela, gbc);
        gbc.gridx = 1;
        panelCampos.add(txtEscuela, gbc);
        ...
        btnRegistrar.addActionListener(e -> {
            ...
            int confirm = JOptionPane.showConfirmDialog(this, "¿Desea registrar al niño?", "Confirmar registro", JOptionPane.YES_NO_OPTION);
            if (confirm == JOptionPane.YES_OPTION) {
                String folio = String.format("N-%04d", contadorNinos++);
                Ninoo ninoo = new Ninoo(nombre, paterno, materno, edad, tutor, telefono, escuela, folio);
                listaNinos.add(ninoo);
                JOptionPane.showMessageDialog(this, "Registro exitoso", "Registrado", JOptionPane.INFORMATION_MESSAGE);
                txtNombre.setText(""); txtPaterno.setText(""); txtMaterno.setText(""); txtEdad.setText("");
                txtTutor.setText(""); txtTelefono.setText(""); txtEscuela.setText("");
            }
        });

        // Acción botón listar 1
        btnListar1.addActionListener(e -> {
            if (listaNinos.isEmpty()) {
                JOptionPane.showMessageDialog(this, "No hay niños registrados.", "Sin registros", JOptionPane.INFORMATION_MESSAGE);
                return;
            }
            for (Ninoo n : listaNinos) {
                JOptionPane.showMessageDialog(this, n.toString(), "Niño registrado", JOptionPane.INFORMATION_MESSAGE);
            }
        });

        // Acción botón listar 2
        btnListar2.addActionListener(e -> {
            ...
            int seleccion = JOptionPane.showOptionDialog(this, "¿Cómo desea mostrar los registros?", "Orden de listado",
                    JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null, opciones, opciones[0]);

            List<Ninoo> copiaLista = new ArrayList<>(listaNinos);
            if (seleccion == 1) {
                copiaLista.sort(Comparator.comparing(Ninoo::getPaterno, String.CASE_INSENSITIVE_ORDER));
            }
            System.out.println("=== LISTADO DE NIÑOS REGISTRADOS ===");
            for (Ninoo n : copiaLista) {
        ...
        btnBuscar.addActionListener(e -> {
            ...
            String apellidoBuscar = JOptionPane.showInputDialog(this, "Ingrese el apellido paterno del niño:");
            if (apellidoBuscar == null || apellidoBuscar.trim().isEmpty()) return;

            boolean encontrado = false;
            for (Ninoo n : listaNinos) {
                if (n.getPaterno().equalsIgnoreCase(apellidoBuscar.trim())) {
                    JOptionPane.showMessageDialog(this, "¡NIÑO ENCONTRADO!\n" + n.toString(), "Resultado", JOptionPane.INFORMATION_MESSAGE);
                    encontrado = true;
                }
            }
            if (!encontrado) {
                JOptionPane.showMessageDialog(this, "No se encontró ningún niño con ese apellido.", "No encontrado", JOptionPane.WARNING_MESSAGE);
            }
        });
